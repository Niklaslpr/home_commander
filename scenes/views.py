        ...
        if data["action"] == "create":
            if "group_id" in data.keys() and (
                ...
                if "attributes" in data.keys():
                    if "name" in data["attributes"]:
                        response = helper.create_scene_in_deconz(int(data["group_id"]), data["attributes"]["name"])
                        if response is not None and isinstance(response, dict):
                            print(response)
                            new_scene = Scene(scene_id=response["response"][0]["success"]["id"].__str__(),
                                              name=data["attributes"]["name"], group_id=data["group_id"])
                            new_scene.save()
                ...
                return JsonResponse({"error": "no group id specified or wrong data type"})
            ...
            if "group_id" in data.keys() and (
                ...
                if "attributes" in data.keys():
                    ...
                    if "scene_id" in data.keys() and (
                        ...
                        "scene_id"].isnumeric()):
                        response = helper.update_scene_deconz(int(data["group_id"]), int(data["scene_id"]),
                                                              **request_data)
                    ...
                    if "scene_id" in data.keys() and (
                            isinstance(data["scene_id"], int) or isinstance(data["scene_id"], str) and data[
                        "scene_id"].isnumeric()):
                        scene = Scene.objects.get(scene_id__exact=data["scene_id"], group_id__exact=data["group_id"])
                        if "icon" in data["features"].keys():
                            scene.icon = data["features"]["icon"]

                        scene.save()

                        return JsonResponse({"success": "changed"})
                    else:
                        return JsonResponse({"error": "no scene id specified or wrong data type"})
                    ...
                    if "scene_id" in data.keys() and (
                        ...
                        if "all" in data["states"].keys() and data["states"]["all"] == "store_current_states":
                            response = helper.update_scene_state_deconz(int(data["group_id"]), int(data["scene_id"]))
                            print("HALLO HIER",response[0])
                            return JsonResponse(response[0])
                        elif "on" in data["states"].keys() and data["states"]["on"].__str__() == "y":
                            response = helper.activate_scene(int(data["group_id"]), data["scene_id"])
                            return JsonResponse(response)
                        else:
            ...
            if "group_id" in data.keys() and (
                ...
                if "scene_id" in data.keys() and (
                        isinstance(data["scene_id"], int) or isinstance(data["scene_id"], str) and data[
                    "scene_id"].isnumeric()):
                    response = helper.delete_scene_deconz(int(data["group_id"]), int(data["scene_id"]),
                                                          request.user.get_username())
                                                          
                    scene = Scene.objects.get(group_id__exact=data["group_id"], scene_id__exact=data["scene_id"])
                    scene.delete()
